...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    //add to cart
        if(event.target.classList.contains("shop-item-button")){
            ...
            axios.post('http://43.205.255.129:4000/cart',{'productId':parentId.substring(1)})
            .then((response)=>{
    ...
    if (event.target.innerText=='REMOVE'){
        axios.post('http://43.205.255.129:4000/cart-delete-item',{'productId':event.target.parentNode.parentNode.id.substring(8)})
        .then(response=>{
    ...
    if (event.target.className=='purchase-btn'){
        if (parseInt(document.querySelector('.cart-number').innerText) === 0){
            ...
            axios.post('http://43.205.255.129:4000/order-place')
            .then((response)=>{
                notifyUser(response.data.message);
                const cart_items = document.querySelector('#cart .cart-items');
                cart_items.innerHTML = ""
...
function displayProducts(queryParams=''){
    axios.get(`http://43.205.255.129:4000/products/${queryParams}`)
    .then(result=>{
...
function displayInCart(queryParams=''){
        axios.get(`http://43.205.255.129:4000/cart/${queryParams}`)
        .then(response=>{
            const cart_items = document.querySelector('#cart .cart-items');
            cart_items.innerHTML='';
